 ...
 *
 * Extracted from index.ts to keep the entry point clean (re-exports only).
 * Each inline step follows the Occam's Razor principle: only code that
 * Claude cannot do (regex, semver, API calls) lives here.
 */
...
import type { Detection, ScanResult } from './analyzer/types.js';
import {
...
export async function analyze(options: AnalyzeOptions): Promise<AnalyzeResult> {
  ...
  for (let i = 0; i < scanResult.detections.length; i++) {
    ...
    // Affected files: count of unique files touched by this detection
    // (single detection = 1 file; AI agent can aggregate across detections)
    const affectedFiles = 1;
    ...
    try {
      ...
      updatePlan = {
        ...
        summary: {
          ...
          totalAffectedFiles: updates.reduce((sum, u) => sum + u.affectedFiles, 0),
        },
      };
    } catch {
